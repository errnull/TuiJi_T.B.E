...
{
    if (TJStringIsNull(blockIdentifier)) {
        return;
    }
    if (self.repeatActionInfoDict.count == 0) {
        return;
...
+ (NSString *)repeatDoAction:(dispatch_block_t)block withDelay:(NSTimeInterval)delaySecond withRepeatDuration:(NSTimeInterval)duration
{
    ...
    NSString *blockIdentifier = TJStringCurrentTimeStamp;
...
{
    if (GJCFCheckObjectNull(aLayer) || TJStringIsNull(path) || GJCFCheckKeyValueHasNull(fromValue, toValue)) {
        return;
    }
...
{
    if (GJCFCheckObjectNull(aLayer) || TJStringIsNull(path) || GJCFCheckObjectNull(value)) {
        return;
    }
