//
//  GJGCChatFriendVideoMessageCell.m
//  TuiJi_T.B.E
//
...
#import "GJGCChatFriendVideoMessageCell.h"

@interface GJGCChatFriendVideoMessageCell()

@property (nonatomic, weak) UIImageView *videoImageView;

@end

@implementation GJGCChatFriendVideoMessageCell
{
    NIMVideoObject *_currentVideoObject;
}
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        UIImageView *videoImageView = [TJUICreator createImageViewWithName:@"chat_videoPlay" size:CGSizeMake(40, 40)];
        _videoImageView = videoImageView;
        [self.bgshadowView addSubview:videoImageView];
...
{
    if (self.delegate && [self.delegate respondsToSelector:@selector(videoMessageCellDidTap:videoObject:)]) {
        [self.delegate videoMessageCellDidTap:self videoObject:_currentVideoObject];
    }
}
...
- (void)setContentModel:(GJGCChatContentBaseModel *)contentModel
{
    ...
    _currentVideoObject = chatContentModel.videoObject;
    
    self.contentSize = CGSizeMake(200, 200);
    self.contentImageView.gjcf_size = self.contentSize;
    
    self.contentImageView.image = [UIImage imageNamed:chatContentModel.videoObject.coverPath];
    ...
    _videoImageView.center = self.contentImageView.center;
}
