...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier
{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.bgshadowView addSubview:self.contentLabel];
        
        self.textRenderCacheImageView = [[UIImageView alloc]initWithFrame:self.contentLabel.frame];
        [self.bgshadowView addSubview:self.textRenderCacheImageView];
...
- (void)setContentModel:(GJGCChatContentBaseModel *)contentModel
{
    ...
    if (renderCacheImage) {
        ...
        self.bgshadowView.gjcf_height = renderHeight;
        self.bgshadowView.gjcf_width = self.textRenderCacheImageView.gjcf_width + 2*self.contentInnerMargin + 5.5 + 20;
        
        if (chatContentModel.isFromSelf) {
            self.textRenderCacheImageView.gjcf_right = self.bgshadowView.gjcf_width - 5.5 - self.contentInnerMargin - 12;
        }else{
            self.textRenderCacheImageView.gjcf_left = self.contentInnerMargin + 5.5 + 12;
        }
        
        [self adjustContent];
        self.textRenderCacheImageView.gjcf_centerY = self.bgshadowView.gjcf_height/2;
        
    }else{
        
        CGFloat textHeight = self.contentLabel.gjcf_height + 2*self.contentInnerMargin;
        textHeight = MAX(textHeight, 40);
        self.bgshadowView.gjcf_height = textHeight;
        self.bgshadowView.gjcf_width = self.contentLabel.gjcf_width + 2*self.contentInnerMargin + 5.5 + 20;
        
        if (chatContentModel.isFromSelf) {
            self.contentLabel.gjcf_right = self.bgshadowView.gjcf_width - 5.5 - self.contentInnerMargin - 12;
        }else{
            self.contentLabel.gjcf_left = self.contentInnerMargin + 5.5 + 12;
        }
        
        [self adjustContent];
        self.contentLabel.gjcf_centerY = self.bgshadowView.gjcf_height/2;
