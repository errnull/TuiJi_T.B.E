...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [self.bgshadowView addSubview:self.titleLabel];
...
{
    UIImage *originImage = [UIImage imageNamed:@"群成员响应-bg"];
...
- (void)setContentModel:(GJGCChatContentBaseModel *)contentModel
{
    ...
    self.bgshadowView.gjcf_width = self.titleLabel.gjcf_width + 2*self.contentInnerMargin + 20;
    
    [self adjustContent];
    
//    self.bgshadowView.image = [self backgroundImage];
//    self.bgshadowView.highlightedImage = nil;
    
    if (chatContentModel.isFromSelf) {
        self.bgshadowView.gjcf_right = self.bgshadowView.gjcf_right + self.headView.width * 0.5;
        self.nameLabel.gjcf_right = self.bgshadowView.gjcf_right;
    }else{
        self.bgshadowView.gjcf_left = self.bgshadowView.gjcf_left + 3;
        self.nameLabel.gjcf_left = self.bgshadowView.gjcf_left;
    }
    
    self.titleLabel.gjcf_centerY = self.bgshadowView.gjcf_height/2;
}
