...
- (void)layoutSubviews
{
    ...
    [self.contentScrollView.subviews enumerateObjectsUsingBlock:^(UIView *subView, NSUInteger idx, BOOL *stop) {
        ...
        if ([subView.class isSubclassOfClass:[GJGCChatInputExpandMenuPanelItem class]]) {
            ...
            item.gjcf_top = (rowIndex + 1) * self.itemToPanelMargin + rowIndex * itemHeight - 10;
...
{    
    ...
    self.contentScrollView.backgroundColor = TJColorGrayBg;
...
- (void)initMenuItems
{
    if ([self.delegate respondsToSelector:@selector(menuPanelRequireCurrentConfigData:)] && self.delegate) {
        ...
        [dataSourceItems enumerateObjectsUsingBlock:^(NSDictionary *item, NSUInteger idx, BOOL *stop) {
            ...
            GJGCChatInputExpandMenuPanelItem *panelItem = [GJGCChatInputExpandMenuPanelItem itemWithTitle:itemTitle
                                                                                      withIconImageNormal:iconImage
                                                                                   withIconImageHighlight:iconHighlightImage
                                                                                           withActionType:actionType
                                                                                             withTapBlock:^(GJGCChatInputExpandMenuPanelItem *item) {
