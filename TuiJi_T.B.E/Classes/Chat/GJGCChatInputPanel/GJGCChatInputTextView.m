...
- (void)initSubViewsWithFrame:(CGRect)frame
{
    ...
    [self.recordButton setTitleColor:TJColorGray forState:UIControlStateNormal];
...
- (void)observeAppendFocusOnOther:(NSNotification *)noti
{
    ...
    if (TJStringIsNull(self.textView.text)) {
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([text isEqualToString:@"\n"]) {
        ...
        /* 是否纯空格 */
        BOOL isAllWhiteSpace = TJStringIsAllWhiteSpace(self.textView.text);
        if (isAllWhiteSpace) {
//            [[NSNotificationCenter defaultCenter] postNotificationName:GJGC_NOTIFICATION_TOAST_NAME object:nil userInfo:@{@"message":@"不允许发送空信息"}];
        ...
        if (self.finishInputBlock) {
            ...
            if (!TJStringIsNull(textView.text)) {
...
{
    return TJStringIsNull(self.textView.text);
}
