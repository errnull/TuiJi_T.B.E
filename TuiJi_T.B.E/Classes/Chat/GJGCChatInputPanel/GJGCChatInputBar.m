...
- (void)initSubViews
{
    ...
    [self setBackgroundColor:TJColorWhite];
    
    /* 录音按钮 */

    UIImage *recordIcon = GJCFQuickImage(@"icon_toolview_voice");
    UIImage *keybordIcon = GJCFQuickImage(@"icon_toolview_keyboard");
    ...
    if (GJCFSystemiPhone6Plus) {
        ...
        self.recordAudioBarItem.frame = CGRectMake(self.itemToBarMargin, 8,32, 32);
        
    }else{
        
        self.recordAudioBarItem.frame = CGRectMake(self.itemToBarMargin, 0,32, 32);
        self.recordAudioBarItem.gjcf_centerY = self.barHeight/2+6;
    }
    ...
    /* 展开面板按钮 */
    UIImage *extendIcon = GJCFQuickImage(@"icon_toolview_add");
    self.openPanelBarItem = [[GJGCChatInputBarItem alloc]initWithSelectedIcon:keybordIcon withNormalIcon:extendIcon];
    if (GJCFSystemiPhone6Plus) {
        self.openPanelBarItem.frame = CGRectMake(0, 8,32, 32);
    }else{
        self.openPanelBarItem.frame = CGRectMake(0, 0,32, 32);
    ...
    /* 输入文本 */
    self.inputTextView = [[GJGCChatInputTextView alloc]initWithFrame:CGRectMake(self.recordAudioBarItem.gjcf_right + self.itemMargin ,0,GJCFSystemScreenWidth - 35 * 3 - 2*self.itemToBarMargin - self.itemMargin * 3 +10, 32)];
    ...
    /* 表情按钮 */
    UIImage *emojiIcon = GJCFQuickImage(@"icon_toolview_emotion");
    self.emojiBarItem = [[GJGCChatInputBarItem alloc]initWithSelectedIcon:keybordIcon withNormalIcon:emojiIcon];
    if (GJCFSystemiPhone6Plus) {
        self.emojiBarItem.frame = CGRectMake(self.inputTextView.gjcf_right+self.itemMargin, 8,32, 32);
    }else{
        self.emojiBarItem.frame = CGRectMake(self.inputTextView.gjcf_right+self.itemMargin, 0,32, 32);
