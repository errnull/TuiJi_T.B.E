...
- (instancetype)initWithPanelDelegate:(id<GJGCChatInputPanelDelegate>)aDelegate;
{
    if (self = [super init]) {
        ...
        _panelIndentifier = [NSString stringWithFormat:@"GJGCChatInputPanel_%@",TJStringCurrentTimeStamp];
...
- (void)initSubViews
{
    ...
    /* 表情面板 */
    self.emojiPanel.backgroundColor = TJColorGrayBg;
    ...
    self.menuPanel.backgroundColor = TJColorGrayBg;
...
{
    /* 是否纯空格 */
    BOOL isAllWhiteSpace = TJStringIsAllWhiteSpace(self.messageDraft);
    if (isAllWhiteSpace) {
