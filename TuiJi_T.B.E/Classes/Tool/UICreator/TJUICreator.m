...
@implementation TJUICreator

//For Line
//绘制线
+ (UIView *)createLineWithSize:(CGSize)size bgColor:(UIColor *)color{
    
    UIView *line = [[UIView alloc]initWithFrame:TJRectFromSize(size)];
...
{
    UIView *view = [[UIView alloc] initWithFrame:TJRectFromSize(size)];
...
{
    UIView *view = [TJUICreator createViewWithSize:size
                                           bgColor:bgColor
                                            radius:cornerRadius];
...
+ (UILabel *)createLabelWithSize:(CGSize)size
                         ...
                         bgColor:(UIColor *)bgColor
{
    UILabel *label = [[UILabel alloc] initWithFrame:TJRectFromSize(size)];
    ...
    label.font = [UIFont fontWithName:TJFontName size:label.font.pointSize];
...
+ (UIView *)createTitleViewWithSize:(CGSize)size
                     ...
                     sysFontSize:(int)fontSize
{
    return [self createTitleViewWithSize:size text:text textColor:TJColorWhite sysFontSize:fontSize];
}
...
+ (UIView *)createTitleViewWithSize:(CGSize)size
                        ...
                        sysFontSize:(int)fontSize
{
    UIButton *titleView = [TJUICreator createButtonWithTitle:text
                                                        size:size
                                                  titleColor:color
                                                        font:TJFontWithSize(fontSize)
    ...
    titleView.backgroundColor = TJColorClear;
                             ...
                             action:(SEL)action
{
    ...
    button.frame = TJRectFromSize(size);
...
{
    UIImageView *imageView = [[UIImageView alloc] initWithFrame:TJRectFromSize(size)];
...
{
    UITextField *textField = [[UITextField alloc] initWithFrame:TJRectFromSize(size)];
...
{
    UITextView *textView = [[UITextView alloc] initWithFrame:TJRectFromSize(size)];
    if (atrributeString) {
...
{
    UITableView *tableView = [[UITableView alloc] initWithFrame:TJRectFromSize(size) style:style];
    if (height == 0) {
        height = 44;
    }
...
{
    // btn
    UIButton *btn = [[UIButton alloc] initWithFrame:TJRectFromSize(size)];
...
//For TJAnimTextField
+ (TJAnimTextField *)createAnimTextFieldWithSize:(CGSize)size
                                            Font:(UIFont *)font
                                       textColor:(UIColor *)textColor
                                 backgroundColor:(UIColor *)backgroundColor
                                     borderStyle:(UITextBorderStyle)borderStyle
                                   ...
                                   returnKeyType:(UIReturnKeyType)returnKeyType
                                        delegate:(id<UITextFieldDelegate>)delegate
{
    TJAnimTextField *textView = [[TJAnimTextField alloc]initWithFrame:TJRectFromSize(size)];
    ...
    textView.placeholderColor = TJColorlightGray;
...
//For TJInPutView
+ (TJInPutView *)createInPutViewWithsize:(CGSize)size
                                    Type:(TJInPutViewType)inPutViewType
{
    TJInPutView *inPutView = [[TJInPutView alloc] initWithsize:size Type:inPutViewType];
    return inPutView;
}

//For TJAgreeView
+ (TJAgreeView *)createAgreeViewWithSize:(CGSize)size
                                  target:(id)target
                                  action:(SEL)action
{
    TJAgreeView *agreeView = [[TJAgreeView alloc] initWithsize:size
                                                        target:target
                                                        action:action];
    return agreeView;
}
...
//For
+ (TJVerityButton *)createVerityButtonWithSize:(CGSize)size
                                        ...
                                        action:(SEL)action
{
    TJVerityButton *verityButton = [[TJVerityButton alloc] initWithFrame:TJRectFromSize(size)
                                                             setTitle:title
                                                                 font:font
                                                                 time:time
                                                               corner:corner
...
+ (TJRegionView *)createRegionViewWithRegionStr:(NSString *)regionStr
                                           font:(UIFont *)font
{
    regionStr = TJStringIsNull(regionStr) ? @"中国_&CHI" : regionStr;
    TJRegionView *regionView = [[TJRegionView alloc] initWithRegionStr:regionStr font:font];
    
    return regionView;
}

+ (TJKeyValueTextLabel *)createKeyValueTextLabelWithSize:(CGSize)size
...
{
    TJKeyValueTextLabel *keyValueTextLabel = [[TJKeyValueTextLabel alloc] initWithSize:size
                                                                               textKey:textKey
                                                                             textValue:textValue
                                                                              keyColor:keyColor
