...
@interface TJProfileViewController()<UIScrollViewDelegate,UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate,UICollectionViewDataSource,UIActionSheetDelegate, TJMyInfoViewDelegate, TJMyTuiTimeLineViewCellDelegate,TJSingleListSelectorDelegate, NIMSystemNotificationManagerDelegate>

@property (nonatomic, weak) TJMyInfoView *headerView;
...
@implementation TJProfileViewController
{
    MCCustomBar *_myTimeLineBar;    //动态
    MCCustomBar *_mySettingBar;     //文章
...
{
    TJMyInfoView *headView = [[TJMyInfoView alloc] initWithFrame:CGRectMake(0, 0, TJWidthDevice, 1)];
...
-(void)createSectionView{
    ...
    CGFloat ControlBarWidth = TJWidthDevice/3;
    ...
    //文章bar
    _mySettingBar = [[MCCustomBar alloc] initWithCount:@"0" size:barSize andImageName:@"profilr_setting" selectedImageName:@"profilr_setting_h"];
    [_mySettingBar addTarget:self action:@selector(userSettingViewClick:) forControlEvents:UIControlEventTouchUpInside];
    _mySettingBar.tag = 1;
    _mySettingBar.gjcf_origin = CGPointMake(ControlBarWidth, ControlBarY);
    [_sectionView addSubview:_mySettingBar];
    //更多bar
    ...
    _myPictureBar.tag = 2;
    _myPictureBar.gjcf_origin = CGPointMake(2*ControlBarWidth, ControlBarY);
...
-(void)createTableScrollView{
    ...
    //我的推文
    TJMyTuiTimeLineView *myTuiTimeLineView = [[TJMyTuiTimeLineView alloc] initWithFrame:CGRectMake(0, 0, TJWidthDevice, tableScrollHeight - TJHeightTabBar)];
    ...
    //我的图片
    TJMyTuiPictureView *myTuiPictureView = [[TJMyTuiPictureView alloc] initWithFrame:CGRectMake(TJWidthDevice*2, 0, TJWidthDevice, tableScrollHeight - TJHeightTabBar)];
...
//            } failure:^(NSError *error) {}];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1*NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [self.myTuiTimeLineView.myTimeLineTableView.mj_header endRefreshing];
    });
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    myTimeLine.headImage = TJUserInfoCurrent.uPicture;
...
- (void)myInfoView:(TJMyInfoView *)myInfoView attentionViewClick:(UIButton *)sender{
    TJMyAttentionTVC *myAttentionTVC = [[TJMyAttentionTVC alloc] init];
    
    myAttentionTVC.hidesBottomBarWhenPushed = YES;
    [(TJNavigationController *)self.navigationController pushToLightViewController:myAttentionTVC animated:YES];
}

- (void)myInfoView:(TJMyInfoView *)myInfoView fansViewClick:(UIButton *)sender{
    TJMyFansTVC *myFansTVC = [[TJMyFansTVC alloc] init];
    [_headerView hideRedPoint];
    self.tabBarItem.badgeValue = 0;
    myFansTVC.hidesBottomBarWhenPushed = YES;
    [(TJNavigationController *)self.navigationController pushToLightViewController:myFansTVC animated:YES];
